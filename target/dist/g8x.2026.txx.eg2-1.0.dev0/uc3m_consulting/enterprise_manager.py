...
class EnterpriseManager:
    ...
    def validate_cif(cif: str):
        ...
        #added to get rid of Unused argument 'cif' (unused-argument)
        cif=True
        return cif

    @staticmethod
    def register_document(input_file: str):
        """applied logic for GE2b project  """
        def is_hex(val):
            try:
                int(val, 16)
                return True
        ...
        try:
            ...
            raise EnterpriseManagementException("Input file not found") from ec
        except json.JSONDecodeError as ec:
            raise EnterpriseManagementException("The file is not JSON formatted") from ec
        ...
        if not (is_hex(project_id) and len(project_id) == 32
                and filename[:8].isalnum() and
                filename.endswith(valid_extensions) and filename[8] == "."):
